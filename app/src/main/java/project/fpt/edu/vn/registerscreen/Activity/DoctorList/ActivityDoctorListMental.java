...
public class ActivityDoctorListMental extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtListName.setText("Danh sách bác sĩ bệnh tâm thần");
        socketApplication.getSocket().emit("patient_load_doctor","3");

        adapter = new ListDoctorAdapter(
                ActivityDoctorListMental.this,
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Intent intent = new Intent(ActivityDoctorListMental.this, DoctorProfileActivity.class);
    ...
    private void Logout(){
        ...
        finish();
        startActivity(new Intent(ActivityDoctorListMental.this, LoginActivity.class));
    }
    private void doBindService() {
        bindService(new Intent(ActivityDoctorListMental.this, SocketServiceProvider.class), socketConnection, Context.BIND_AUTO_CREATE);
        mIsBound = true;
    }
    @Override
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onMessageEvent(EventReloadDoctorOnline3 event) {
        Log.d("ListDoctorOnline3","True");
        arrayDoctorOnline.clear();
        for(int i=0;i< event.getArrayDoctor().size();i++){
            if(event.getArrayDoctor().get(i).getOnline_status().equalsIgnoreCase("online")) {
                arrayDoctorOnline.add(event.getArrayDoctor().get(i));
