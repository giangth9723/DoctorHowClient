...
import project.fpt.edu.vn.registerscreen.FemaleListAdapter;
...
public class ActivityFemaleExaminationHistory extends AppCompatActivity {
    ArrayList<Emr_female> arrayExam = new ArrayList<Emr_female>();
    FemaleListAdapter adapterExam;
    ListView lstView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_female_examination_history);
        onCreateSocket();
        anhXa();
        socketApplication.getSocket().emit("patient_get_emr_female",session.getPatient().getPatient_id());
        adapterExam = new FemaleListAdapter(
        ...
        imgBtnBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(ActivityFemaleExaminationHistory.this,ActivityFemaleMenu.class));
                finish();
            }
        });
        ...
        lstView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Emr_female e = arrayExam.get(i);
                Bundle bundle = new Bundle();
                bundle.putSerializable("emr_female",e);
                Intent intent = new Intent(ActivityFemaleExaminationHistory.this,ActivityFemaleContent.class);
                intent.putExtra("emr_female",bundle);
    ...
    private void doBindService() {
        bindService(new Intent(ActivityFemaleExaminationHistory.this, SocketServiceProvider.class), socketConnection, Context.BIND_AUTO_CREATE);
        mIsBound = true;
    }
    @Override
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onEvent(EventGetEmrFemale event){
        arrayExam.clear();
        for(int i = 0;i<event.getEmr_femaleArrayList().size();i++){
            arrayExam.add(event.getEmr_femaleArrayList().get(i));
        }
        Log.d("EmrDermatology1",event.getEmr_femaleArrayList().get(0).getDoctor_name());
